...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), allowed_classes=list(read_class_names(cfg.YOLO.CLASSES).values()), show_label=True):
    ...
    for i in range(num_boxes[0]):
        ...
        if class_name not in allowed_classes:
            ...
            handle_coords.append((coor0, coor1, coor2, coor3))
        elif class_name==obj:
            color = get_color_name(image, coor1, coor0, coor3, coor2)
            if color==adjective:
                bbox_color = colors[class_ind]
                bbox_thick = int(0.6 * (image_h + image_w) / 600)
                
                c1, c2 = (coor1, coor0), (coor3, coor2)
                bottle_cord= (coor0, coor1, coor2, coor3)
    ...
    if bottle_detected==True:
        min_dist=100000
        handle=tuple()
        for index, handle_cord in enumerate(handle_coords):
            dist = math.sqrt( ((handle_cord[1]-bottle_cord[1])**2)+((handle_cord[2]-bottle_cord[1])**2) )   
            if(dist<=min_dist):
                min_dist = dist
                handle=handle_cord
        ...
        c1, c2 = (int(handle[1]), int(handle[0])), (int(handle[3]), int(handle[2]))
        cv2.rectangle(image, c1, c2, bbox_color, bbox_thick)
        if show_label:
            bbox_mess = '%s: %.2f' % (classes[class_ind], score)
            t_size = cv2.getTextSize(bbox_mess, 0, fontScale, thickness=bbox_thick // 2)[0]
        ...
        diff_y = (224-(int(handle[2])-int(handle[0])))/2
        diff_x = (224-(int(handle[3])-int(handle[1])))/2
        
        x_start=int(handle[1]-diff_x)
        x_end=int(handle[3]+diff_x)
        
        y_start=int(handle[0]-diff_y)
        y_end=int(handle[2]+diff_y)
        image_g= image[x_start:x_end , y_start:y_end , :]
        image_g_np=np.array(image_g)
        image_g_pp = preprocess_input(image_g_np)
        image_expanded =np.expand_dims(image_g_pp,0)
        
        
        loaded_model = tf.keras.models.load_model('./data/weights_grasp/')
        y=loaded_model.predict(image_expanded)
        image=draw_grasp(image,y[0], x_start, y_start)
...
def draw_grasp(image, grasp, x_start, y_start):
    centre = [grasp[0]+x_start,grasp[1]+y_start]
    height, width, theta = grasp[2]*2, grasp[3]*2, grasp[4]
    ...
    return draw_bbox1(image,bbox)

def draw_bbox1(image, bbox):
    img = image.copy()
...
def get_color_name(image, coor1, coor0, coor3, coor2):
    center_x= (coor3+coor1)/2
    center_y= (coor2+coor0)/2
    R,G,B = image[int(center_y),int(center_x)]
